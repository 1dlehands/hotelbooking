...
describe Booking do
  before do
    @booking = Booking.new(user_id: 1, room_id: 1, start_date: 9.days.from_now, stop_date: 11.days.from_now)
  end
  ...
  describe "is not valid if start date is before today" do
    before { @booking.start_date = 1.day.ago }
    it { should_not be_valid }
  end
  
  describe "is not valid if the duration is less than one day" do
    before { @booking.stop_date = @booking.start_date }
    it { should_not be_valid }
  end
